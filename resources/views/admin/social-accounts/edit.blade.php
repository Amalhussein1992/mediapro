...
@section('header', 'Edit Social Account')

@section('content')
<div class="max-w-4xl">
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.social-accounts.update', $socialAccount) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                >
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ old('user_id', $socialAccount->user_id) == $user->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                >
                    ...
                    <option value="facebook" {{ old('platform', $socialAccount->platform) == 'facebook' ? 'selected' : '' }}>Facebook</option>
                    <option value="twitter" {{ old('platform', $socialAccount->platform) == 'twitter' ? 'selected' : '' }}>Twitter</option>
                    <option value="instagram" {{ old('platform', $socialAccount->platform) == 'instagram' ? 'selected' : '' }}>Instagram</option>
                    <option value="linkedin" {{ old('platform', $socialAccount->platform) == 'linkedin' ? 'selected' : '' }}>LinkedIn</option>
                    <option value="tiktok" {{ old('platform', $socialAccount->platform) == 'tiktok' ? 'selected' : '' }}>TikTok</option>
                </select>
                @error('platform')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input
                    ...
                    id="account_name"
                    name="account_name"
                    value="{{ old('account_name', $socialAccount->account_name) }}"
                    class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
            ...
            <div class="mb-4">
                ...
                <input
                    ...
                    id="account_id"
                    name="account_id"
                    value="{{ old('account_id', $socialAccount->account_id) }}"
                    class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
            ...
            <div class="mb-4">
                ...
                >{{ old('access_token', $socialAccount->access_token) }}</textarea>
                @error('access_token')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                >{{ old('refresh_token', $socialAccount->refresh_token) }}</textarea>
                @error('refresh_token')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
