    ...
    <div class="bg-white rounded-xl shadow-lg p-8 border border-gray-100">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-gray-800 flex items-center">
                ...
                تعديل الصفحة: {{ $page->title }}
            </h2>
            <p class="text-gray-600 mt-2 mr-15">قم بتعديل معلومات الصفحة أدناه</p>
        </div>
            ...
            <div class="mb-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" id="icon" value="{{ old('icon', $page->icon) }}"
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="content_ar" id="content_ar" rows="8"
                            ...
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('content_ar') border-red-500 @enderror">{{ old('content_ar', $page->content_ar) }}</textarea>
                        @error('content_ar')
                            <p class="text-red-500 text-sm mt-2 flex items-center">
                                <i class="fas fa-exclamation-circle ml-2"></i>
                                {{ $message }}
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea name="meta_description" id="meta_description" rows="2"
                            ...
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('meta_description', $page->meta_description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="meta_description_ar" id="meta_description_ar" rows="2"
                            ...
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('meta_description_ar', $page->meta_description_ar) }}</textarea>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="section" id="section" required
                            ...
                            <option value="main" {{ old('section', $page->section) == 'main' ? 'selected' : '' }}>الصفحات الرئيسية (Main)</option>
                            <option value="features" {{ old('section', $page->section) == 'features' ? 'selected' : '' }}>الميزات (Features)</option>
                            <option value="company" {{ old('section', $page->section) == 'company' ? 'selected' : '' }}>الشركة (Company)</option>
                            <option value="resources" {{ old('section', $page->section) == 'resources' ? 'selected' : '' }}>الموارد (Resources)</option>
                            <option value="legal" {{ old('section', $page->section) == 'legal' ? 'selected' : '' }}>القانونية (Legal)</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            <option value="published" {{ old('status', $page->status) == 'published' ? 'selected' : '' }}>منشور (Published)</option>
                            <option value="draft" {{ old('status', $page->status) == 'draft' ? 'selected' : '' }}>مسودة (Draft)</option>
                        </select>
                    </div>
                </div>
            </div>
